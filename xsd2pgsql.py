    ...
    for x in el.findall(ns + 'element'):
        ...
        if not rez[0]:
            ...
            if not pgType and fail:
                ...
                if not cols:
                    cols = "{}{} {} {} {}".format("/*" + (x.get('name') or x.get('ref')) + "*/ " if relations else "",
                                                  colName, pgType, notNull, ForeignKey if relations else "")
                else:
                    cols += ",{}{} {} {} {}".format("/*" + (x.get('name') or x.get('ref')) + "*/ " if relations else "",
                                                    colName, pgType, notNull, ForeignKey if relations else "")
    if cols:
        sql += """CREATE TABLE {}{}({});""".format(pg_normalize(parent) if normalize else parent,
                                                   " /*" + parent + "*/ " if relations else " ", cols)

    for elements in ['complexType', 'complexContent', 'sequence', 'extension', 'choice', 'enumeration']:
        for x in el.findall(ns + elements):
            children = True
            rez = look4element(ns, x, x.get('name') or parent, recurse_level + 1, fail=fail, normalize=normalize,
                               relations=relations)
            sql += rez[1] + '\n'
    return (children, sql)
    ...
    if not args.xsd:
        ...
        for f in args.xsd:
            ...
            if args.as_is:
                ...
                norm = True
                result = look4element(XMLS, xsd, pg_normalize(f.name.split('.')[0]), fail=args.fail_on_bad_type,
                                      normalize=norm, relations=args.relations)
            if result[1]:
